...
from dbscan_chrisjmccormick import MyDBSCAN
...
log.info('Running my implementation...')
...
log.info('Runing scikit-learn implementation...')
...
for i in range(0, len(skl_labels)):
    if not skl_labels[i] == my_labels[i]:
        log.info('Scikit learn:', skl_labels[i], 'mine:', my_labels[i])
        num_disagree += 1

if num_disagree == 0:
    log.info('PASS - All labels match!')
    log.debug(my_labels)
else:
    log.info('FAIL -'+str(num_disagree)+ 'labels don\'t match.'  )

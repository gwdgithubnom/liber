...
def replace_threshold(threshold_pile, n):
    for x, y in enumerate(threshold_pile):
        if y > n:
            threshold_pile[x] = n
    return threshold_pile


def pile_brother(index_id, id_index, distance, distance_c, rho_id, pile, threshold, state=False):
    pre = 0
    next = pile
    # log.fatal(pile)
    # rho_id = rho_function(index_id, distance, distance_c=distance_c)
    rho_threshold = rho_id.copy(deep=True)
    rho_threshold[pile] = len(pile)
    pile = list(set(pile))
    threshold_pile = replace_threshold(rho_threshold, len(pile))
    while (len(pile) != pre):
        pre = len(pile)
        pile_next = []
        next = list(set(next))
        next = sorted(next, key=lambda v: rho_id[v], reverse=True)
        ...
        for l in next:
            ...
            # log.debug(p)
            b = pile_union(pile, p)
            threshold_pile = replace_threshold(threshold_pile, len(p))
            a = len(b)
            if (a > threshold and len(p) < threshold_pile[l]) or (a <= threshold and a >= len(pile)):
                # log.debug(str(threshold_pile_temp)+" brother:\n"+str(next)+"\n"+str(p))
                threshold_pile_temp = len(p)
                pile_next.extend(pile_sub(p, pile))
                pile_next = list(set(pile_next))
                if state == True:
                    break
        next = pile_next
        pile = list(set(pile_union(pile, next)))
...
def outlier_to_pile(outlier, index_id, id_index, distance, distance_c, next_distance_c, border, pile_id, rho_id,
    ...
    while True:
        pile = pile_brother(index_id, id_index, distance, distance_c, rho_id, outlier, threshold, state=False)
        # pile = pile_brother(index_id, id_index, distance, distance_c, rho_id,outlier, threshold, state=False)
        ...
        for ii in range(pile_id_size):
            ...
            if len(jarge) >= jarge_size:
                ...
                jarge_point = list(set(jarge_point))
...
def pile_rho_values(pile, rho_id, rho_id_border, rho_id_border_mean, rho_id_border_ava):
    ...
    if rho_id_border is None or rho_id_border_mean is None or rho_id_border_ava is None:
        for l in pile:
            sort_value = rho_id[l]
        return sort_value

    for l in pile:
        sort_value = rho_id[l] + rho_id_border[l] * 0.8 + rho_id_border_ava[l] * 0.8 + rho_id_border_mean[
                                                                                           l] * 0.8 * 0.8 * 0.8
    return sort_value
...
def pile_function(pile_id, id_index, index_id, data, distance, distance_c, next_distance_c, dataset="default",
    ...
    threshold = 3
    pile = pile_brother(index_id, id_index, distance, distance_c, rho_id, pile, threshold, state=False)
    # pile.sort(key=lambda v:rho_id[v],reverse=True)
    # pile=sorted(pile,key=lambda v:rho_id[v])
    # log.debug((str(len(pile))+"   %%%%%   "+str(pile)))
    ...
    while True:
        ...
        if value == 0:
            ...
            pile_id = pile_id.reset_index(drop=True)
            outlier_list = sorted(outlier_list,
                                  key=lambda v: pile_rho_values(v, rho_id, rho_id_border, rho_id_border_mean,
                                                                rho_id_border_ava), reverse=True)
            ...
            if value == pile_max:
                pile_id = pile_id.append(
                    DataFrame([dict(pile=[i_id], p_id=p_id, size=len(pile), outlier=False)], index=[len(pile_id)]))
            else:
                pile_id = pile_id.append(
                    DataFrame([dict(pile=[i_id], p_id=p_id, size=len(pile), outlier=True)], index=[len(pile_id)]))
        ...
        while len(pile_id) > next:
            ...
            min_len = min(len(pile), len(pre))
            max_len = max(len(pile), len(pre))
            if (min_len < pile_max * 2 and len(intersection) > 3) or len(intersection) >= ((min_len + max_len) * 0.2):
                # or len(intersection)>min_len/3
                # min_len<max_len*0.2
        ...
        rho_set_tag(rho_id_tag, pile)
        pile_id = pile_id.append(
            DataFrame([dict(p_id=p_id, pile=pile, size=len(pile), outlier=outlier)]), ignore_index=True)
...
def pile_pile_function(pile_id, id_index, index_id, data, distance, distance_c, next_distance_c, dataset="default",
                       level="INFO"):
    # 初始化队列

    log.info("running pile_pile_function.. distance_c:" + str(distance_c) + " dataset:" + dataset)
    rho_id = rho_function(index_id, distance, distance_c=distance_c)
    ...
    pile_pile_id = DataFrame([], columns=['p_id', 'pile', 'size', 'outlier'])

    pile_id_size = len(pile_id)
    pile_list = []
    for ii in range(pile_id_size):
        pile_list.append(pile_id.ix[ii, 'pile'])

    pile_pile_id = pile_pile_id.reset_index(drop=True)
    pile_list = sorted(pile_list, key=lambda v: pile_rho_values(v, rho_id, None, None, None), reverse=True)
    pile = pile_list[0]
    threshold = 3
    pile = pile_brother(index_id, id_index, distance, distance_c, rho_id, pile, threshold, state=False)
    rho_set_tag(rho_id_tag, pile)
    if pile is None:
        # pile_id = pile_id.append(DataFrame([dict(pile=-1, size=1)], index=index_id.values))
        return None
    pile_pile_id = pile_pile_id.append(
        DataFrame([dict(p_id=p_id, pile=pile, size=len(pile), outlier=False)], index=[len(pile_pile_id)]))
    # log.debug(pile_id)
    # log.debug("i:"+ str(i))
    # delta_id, data_index = delta_function(id_index, index_id, rho_id, distance)
    static_distance_c = distance_c
    # rho_id = rho_function(index_id, distance, distance_c=distance_c)
    if level is "DEBUG":
        save_show_cluster(index_id, data, distance_c, pile_pile_id, dataset=dataset, level=level,
                          level_info="first outlier")
    p_id = 0
    while p_id < len(pile_list):
        pile = pile_list[p_id]
        p_id += 1
        pile = pile_brother(index_id, id_index, distance, distance_c, rho_id, pile, threshold, state=False)
        value = len(pile)
        if value <= pile_min:
            ...
            if value == pile_max:
                pile_pile_id = pile_pile_id.append(
                    DataFrame([dict(pile=pile, p_id=p_id, size=len(pile), outlier=False)], index=[len(pile_pile_id)]))
            else:
                pile_pile_id = pile_pile_id.append(
                    DataFrame([dict(pile=pile, p_id=p_id, size=len(pile), outlier=True)], index=[len(pile_pile_id)]))
        ...
        # 假设当前是新类
        outlier = False
        pile_pile_id = pile_pile_id.sort_values(by='size', ascending=False)
        while len(pile_pile_id) > next:
            # 寻找下一个可能的堆的合并
            pre = pile_pile_id.loc[next, 'pile']
            intersection = pile_intersection(pile, pre)
            min_len = min(len(pile), len(pre))
            max_len = max(len(pile), len(pre))
            if (min_len < pile_max * 2 and len(intersection) > 3) or len(intersection) >= ((min_len + max_len) * 0.2):
                ...
                state = False
                pile = list(set(pile_union(pile, pre)))
                pile_pile_id = pile_pile_id.drop(next)
                pile_pile_id = pile_pile_id.reset_index(drop=True)
                next = 0
                # log.warn(rho_id)
                rho_set_tag(rho_id_tag, pile)
        ...
        pile_pile_id = pile_pile_id.reset_index(drop=True)
        pile_pile_id = pile_pile_id.sort_values('size', ascending=False)

        if level is "DEBUG":
            save_show_cluster(index_id, data, distance_c, pile_pile_id, dataset=dataset, level=level,
                              level_info=str("after find new cluster:" + str(len(pile_pile_id))))

    log.info("do job on outliers")
    outlier_list = []
    pile_pile_id = pile_pile_id.sort_values(by='size', ascending=False)
    pile_id_size = len(pile_pile_id)
    for ii in range(pile_id_size):
        if pile_pile_id.loc[ii, 'size'] >= pile_max or not pile_pile_id.loc[ii, 'outlier']:
            continue
        elif pile_pile_id.loc[ii, 'outlier']:
            pile_id_pile = pile_pile_id.ix[ii, 'pile']
            pile_pile_id = pile_pile_id.drop(ii)
            outlier_list.append(pile_id_pile)

    pile_pile_id = pile_pile_id.reset_index(drop=True)
    outlier_list = sorted(outlier_list, key=lambda v: pile_rho_values(v, rho_id, rho_id_border, rho_id_border_mean,
                                                                      rho_id_border_ava), reverse=True)
    ...
    for ll in outlier_list:
        # 以pile_min进行评估收敛性
        # log.debug("running outlier...."+str(ll))
        pile_pile_id = outlier_to_pile(ll, index_id, id_index, distance, distance_c, next_distance_c, border,
                                       pile_pile_id, rho_id,
                                       pile_max)
        if level is "DEBUG":
            save_show_cluster(index_id, data, distance_c, pile_pile_id, dataset=dataset, level=level,
    ...
    save_show_cluster(index_id, data, distance_c, pile_pile_id, dataset=dataset, level=level,
                      level_info="final result")
    log.info("end computing pile, in distance_c:" + str(static_distance_c) + ". pile count about:" + str(
        pile_pile_id.shape[0]) + " pile_max:" + str(pile_max) + " pile_min:" + str(pile_min))
    return pile_pile_id
...
def ent_dc_step_by_step(id_index, index_id, data, threshold, distance, distance_c, dataset='none'):
    ...
    while max_distance_c >= distance_c:
        ...
        ee = np.array(e)
        e = _calc_ent(ee / N)
        merge = list([e, distance_c, len(pile_id)])
        threshold = add_row(threshold, merge)
        jarge_now = pre - e
...
def debug_cluster(id_index, index_id, data, distance, distance_c, next_distance_c, dataset='none', level="DEBUG"):
    ...
    # id_index, index_id
    e = _calc_ent(pile_size.values / N)
    save_show_cluster(index_id, data, distance_c, pile_id, dataset, level)
    log.warn(" record the data status: " + str(distance.shape) + " distance_c:" + str(
        distance_c) + " threshold:" + str(e))

...
/*		FileChannel fileChannel=dataInput.getChannel();
		...
		logger.debug(JSONTool.toJson(series));
		try {
			FileOutputStream file = new FileOutputStream(new File(SystemTools.getWebrootpath() + File.separator + "src" + File.separator + "main" + File.separator + "webapp" + File.separator + "data" + File.separator + "asset" + File.separator + "cluster1.json"));
			try {
				file.write(JSONTool.toJson(series).getBytes());
				file.close();
			} catch (IOException e) {
